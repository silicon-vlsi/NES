...
IF NOT ERRORLEVEL 1 (
    ...
    IF NOT ERRORLEVEL 1 (
        echo Container %CONTAINER_NAME% exists. Restart with 
	echo     "docker start %CONTAINER_NAME% "  or remove with 
	echo     "docker rm %CONTAINER_NAME% " if required.
    ) ELSE (
	echo Container does not exist, pulling %DOCKER_USER%/%DOCKER_IMAGE%:%DOCKER_TAG% and creating %CONTAINER_NAME% ... 4GB file .. have patience
